...
SET passager=(SELECT user_id FROM resa WHERE id=$delete_id)
...
set result = sqlpage.send_mail(json_object(
    ...
    'body', $passager||' a annulé l''offre de covoiturage pour '||(SELECT arrivee FROM trajets WHERE id=$trajet_id)||' le '||(SELECT strftime('%d/%m/%Y',jour) FROM trajets WHERE id=$trajet_id)||'.'
));
