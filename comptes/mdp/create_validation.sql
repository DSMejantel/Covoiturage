...
SET courriel=(SELECT courriel FROM user_info WHERE username=:username)
...
select 'alert' as component,
    ...
    case when json_extract($result, '$.status') = 'accepted' then 'Un code de validation a été envoyé à cette adresse : '||$courriel else 'Cet identifiant n''a pas été trouvé dans notre base.' end as description;
...
'create_verification.sql' AS action,
    ...
    WHERE $courriel<>'mairie-de-barjac48@wanadoo.fr';
